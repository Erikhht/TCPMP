...
void* downMix( NeAACDecHandle hDecoder, real_t **input, uint16_t frame_len, uint8_t channel)
{
	...
	a = hDecoder->time_out[hDecoder->internal_channel[0]]; //C
	b = hDecoder->time_out[hDecoder->internal_channel[channel?4:3]]; //S
	c = hDecoder->time_out[hDecoder->internal_channel[channel?2:1]]; //cum

	for (n=0;n<frame_len;++n)
	{
		real_t cum = c[n] + MUL_F(a[n],RSQRT2) + MUL_F(b[n],RSQRT2);
		c[n] = MUL_F(cum, DM_MUL);
	}
