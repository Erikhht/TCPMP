...
typedef struct
{
    uint8_t sfb_cb;
    uint8_t ms_used;
    uint16_t sect_sfb_offset; 
    int16_t scale_factors; /* [0..255] */

} ic_sfb;

typedef struct
{
    uint8_t num_sec; /* number of sections in a group */
    uint8_t window_group_length;
    ic_group_sect sect[MAX_SFB];
	ic_sfb sfb[MAX_SFB+1];
...
typedef struct
{
    ...
    ic_group *group_end;
	ic_group group[MAX_WINDOW_GROUPS];
