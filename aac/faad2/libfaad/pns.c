                                   ...
                                   uint8_t sub)
{
    ...
    for (i = 0; i < size; i++)
    {
        real_t tmp = scale*(real_t)(int32_t)random_int2();
        spec[i] = tmp;
        energy += tmp*tmp;
    }
    ...
    for (i = 0; i < size; i++)
    {
        ...
        real_t tmp = (int32_t)random_int2();
        if (tmp < 0)
...
{
	real_t *spec_left = ics_left->buffer;
	uint8_t sfb, b;
    uint16_t size, offs;
    ...
    uint8_t sub = 0;
	ic_group *gl,*gr = NULL;
	real_t *spec_right = NULL;

	if (!ics_left->noise_used && (!channel_pair || !ics_right->noise_used))
		return;
	...
	{
		gr = ics_right->group;
		spec_right = ics_right->buffer;
	}

    for (gl = ics_left->group; gl < ics_left->group_end; gl++, gr++)
    {
        /* Do perceptual noise substitution decoding */
        for (b = 0; b < gl->window_group_length; b++)
        {
            for (sfb = 0; sfb < ics_left->max_sfb; sfb++)
            {
                if (is_noise(ics_left, gl, sfb))
                {
#ifdef LTP_DEC
                    ...
                    */
                    ...
                    gen_rand_vector(&spec_left[offs],
                        gl->sfb[sfb].scale_factors, size, sub);
                }
                ...
                if (channel_pair)
                {
                    if (is_noise(ics_right, gr, sfb))
                    {
                            ...
                            (ics_left->ms_mask_present == 2))
                        {
                            ...
                            /* Generate random vector */
                            gen_rand_vector(&spec_right[offs],
                                gr->sfb[sfb].scale_factors, size, sub);
                        }
                    }
                }
            } /* sfb */
