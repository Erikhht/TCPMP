...
#ifdef FIXED_POINT
static const real_t pow05_table[] = {
...
void is_decode(ic_stream *ics, ic_stream *icsr, uint16_t frame_len)
{
    uint8_t sfb, b;
    uint16_t i;
#ifndef FIXED_POINT
	...
	ic_group *gr;
	ic_group *g = ics->group;

	if (!icsr->intensity_used)
		return;

    for (gr = icsr->group; gr != icsr->group_end; ++gr, ++g)
    {
        /* Do intensity stereo decoding */
        for (b = 0; b < gr->window_group_length; b++)
        {
            for (sfb = 0; sfb < icsr->max_sfb; sfb++)
            {
                if (is_intensity(icsr, gr, sfb))
                {
#ifdef MAIN_DEC
...
#else
                    exp = gr->sfb[sfb].scale_factors >> 2;
                    frac = gr->sfb[sfb].scale_factors & 3;
#endif
...
#endif
                        if (is_intensity(icsr, gr, sfb) != invert_intensity(ics, g, sfb))
