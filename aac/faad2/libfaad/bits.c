...
void faad_initbits(bitfile *ld, const void *_buffer, const uint32_t buffer_size, int external)
{
    uint32_t tmp;
	...
	if (!external)
	{
		...
		memset((uint8_t*)ld->buffer, 0, (buffer_size+12)*sizeof(uint8_t));
		memcpy((uint8_t*)ld->buffer, _buffer, buffer_size*sizeof(uint8_t));
	}
	else
	{
		ld->external = 1;
		ld->buffer = _buffer;
		memset((uint8_t*)ld->buffer + buffer_size,0,12*sizeof(uint8_t));
	}
