...
int read_metadata_buff (WavpackContext *wpc, WavpackMetadata *wpmd)
{
    ...
    if (!wpc->infile (&wpmd->id, 1, wpc->privdata) || !wpc->infile (&tchar, 1, wpc->privdata))
	return FALSE;
	...
	if (!wpc->infile (&tchar, 1, wpc->privdata))
	    return FALSE;
	...
	if (wpc->infile (wpc->read_buffer, bytes_to_read, wpc->privdata) != (long) bytes_to_read) {
	    wpmd->data = NULL;
	    return FALSE;
	}
