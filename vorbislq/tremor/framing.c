...
static void _ogg_buffer_destroy(ogg_buffer_state *bs){
  ...
  if(bs->shutdown){
    ...
    while(bt){
	  ...
	  else if(b->data)_ogg_free((void*)b->data);
      _ogg_free(b);
    }
    bs->unused_buffers=0;
    while(rt){
...
ogg_buffer *ogg_sync_bufferinext(ogg_sync_state *oy){
  ogg_reference *new=ogg_buffer_alloc(oy->bufferpool,-1);
  if(!oy->fifo_head){
    oy->fifo_head=oy->fifo_tail=new;
  } else{
    oy->fifo_head->next=new;
    oy->fifo_head=new;
  }
  return oy->fifo_head->buffer;
}
...
unsigned char *ogg_sync_bufferin(ogg_sync_state *oy, long bytes){
  ...
  */
  ...
  if(!oy->fifo_head){
    ...
    return (unsigned char*)oy->fifo_head->buffer->data;
  }
  ...
  if(!oy->fifo_head->length){
    ...
    return (unsigned char*)oy->fifo_head->buffer->data+oy->fifo_head->begin;
  }
...
static ogg_uint32_t _checksum(ogg_reference *or, int bytes){
  ...
  while(or){
    const unsigned char *data=or->buffer->data+or->begin;
    post=(bytes<or->length?bytes:or->length);
    for(j=0;j<post;++j,++pos){
      unsigned char v;
	  if (pos>=22 && pos<26)
	    v = 0;
	  else
	    v = data[j];
      crc_reg=(crc_reg<<8)^crc_lookup[((crc_reg >> 24)&0xff)^v];
    }
    bytes-=j;
    or=or->next;
  }
...
long ogg_sync_pageseek(ogg_sync_state *oy,ogg_page *og){
 ...
 sync_fail:
  ...
  while(oy->fifo_tail){
    ...
    const unsigned char *now=oy->fifo_tail->buffer->data+oy->fifo_tail->begin;
    const unsigned char *next=memchr(now, 'O', oy->fifo_tail->length);
