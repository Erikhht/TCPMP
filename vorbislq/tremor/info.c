...
int _vorbis_unpack_comment(vorbis_comment *vc,oggpack_buffer *opb,int eop){
  int i;
  ...
  if(eop && oggpack_read(opb,1)!=1)goto err_out; /* EOP check */
...
int vorbis_synthesis_headerin(vorbis_info *vi,vorbis_comment *vc,ogg_packet *op){
  ...
  if(op){
	...
	return(_vorbis_unpack_comment(vc,&opb,1));
