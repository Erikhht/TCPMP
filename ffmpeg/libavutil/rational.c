...
int av_reduce(int *dst_nom, int *dst_den, int64_t nom, int64_t den, int64_t max){
    ...
    den = ABS(den)/gcd;
    if(nom<=max && den<=max){
        a1= _AVRational((int)nom,(int)den); //Picard
        den=0;
    }
    ...
    while(den){
        ...
        a0= a1;
		a1= _AVRational((int)a2n, (int)a2d); //Picard
        nom= den;
        den= next_den;
    }
...
AVRational av_d2q(double d, int max){
    ...
    int exponent= 0; //!!! FFMAX( (int)(log(ABS(d) + 1e-20)/log(2)), 0);
    int64_t den= ((int64_t)1) << (61 - exponent);
