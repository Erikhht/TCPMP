...
// ifft128 and ifft64 always maps to C code by Picard 12/11/2004
...
#else
#include "imdct_const.h"
#endif

//static void (* ifft128) (complex_t * buf);
//static void (* ifft64) (complex_t * buf);
#define ifft128 ifft128_c
#define ifft64 ifft64_c
...
#ifdef CREATE_CONST
void dumpsample(FILE* f,sample_t* p,const char* name,int n)
{
	int i;
	fprintf(f,"static const sample_t %s[%d] = {",name,n);
	for (i=0;i<n;++i)
	{
		if ((i & 7)==0) fprintf(f,"\n");
		fprintf(f,"0x%08x,",p[i]);
	}
	fprintf(f,"};\n");
}

void dumpcomplex(FILE* f,complex_t* p,const char* name,int n)
{
	int i;
	fprintf(f,"static const complex_t %s[%d] = {",name,n);
	for (i=0;i<n;++i)
	{
		if ((i & 3)==0) fprintf(f,"\n");
		fprintf(f,"{0x%08x,0x%08x},",p[i].real,p[i].imag);
	}
	fprintf(f,"};\n");
}
#endif
