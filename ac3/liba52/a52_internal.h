 ...
 *
 *
 * GCC ARM inline assembly MUL add by Picard 12/04/2004
 *
 */
...
#if defined(ARM) && !defined(_MSC_VER)

// inline ARM assembly from libmad - MPEG audio decoder library
...
{
	int32_t c;
    c=(a & 0xffff)*(b >> 16)+(a >> 16)*(b & 0xffff); 
	return (c >> 14) + (((a >> 16)*(b >> 16)) << 2 );
}
static inline int32_t MUL_L(int32_t a,int32_t b)
{
	int32_t c;
	c=(a & 0xffff)*(b >> 16)+(a >> 16)*(b & 0xffff); 
	return (c >> 10) + (((a >> 16)*(b >> 16)) << 6); 
}
#else
